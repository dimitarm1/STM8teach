...
这段的本意是，当x为零时，判断y的值，如果也为零，则报错。当x不为零时，计算x与y的和并赋值给z，然后以z的地址为参数来调用函数f。
但是，这段代码的实际运行结果与原意相去甚远。由于else始终与同一对括号内最近未匹配的if结合。所以在程序的眼中，这段程序应该是这个样子。
